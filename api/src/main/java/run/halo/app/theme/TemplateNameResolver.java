...
/**
 * The {@link TemplateNameResolver} is used to resolve template name. <code>Halo</code> has a theme mechanism, template
 * files are provided by different themes, so we need a method to determine whether the template file exists in the
 * activated theme and if it does not exist, provide a default template name.
 *
 * @author guqing
 * @since 2.11.0
 */
public interface TemplateNameResolver {
     ...
     */
    Mono<String> resolveTemplateNameOrDefault(ServerWebExchange exchange, String name, String defaultName);
