...
class ControllerBuilderTest {
    ...
    void buildWithNullReconciler() {
        assertThrows(
                ...
                () -> new ControllerBuilder(null, client).build(),
                "Reconciler must not be null");
    }

    @Test
    void buildWithNullClient() {
        assertThrows(IllegalArgumentException.class, () -> new ControllerBuilder(new FakeReconciler(), null).build());
    }
    ...
    void buildTest() {
        assertThrows(
                IllegalArgumentException.class,
                () -> new ControllerBuilder(new FakeReconciler(), client).build(),
    ...
    void invalidMinDelayAndMaxDelay() {
        ...
        assertNotNull(
                fakeBuilder().minDelay(null).maxDelay(Duration.ofSeconds(1)).build());

        assertNotNull(
                fakeBuilder().minDelay(Duration.ofSeconds(1)).maxDelay(null).build());

        assertNotNull(fakeBuilder().minDelay(Duration.ofSeconds(-1)).build());

        assertNotNull(fakeBuilder().maxDelay(Duration.ofSeconds(-1)).build());
    }

    ControllerBuilder fakeBuilder() {
        return new ControllerBuilder(new FakeReconciler(), client).extension(new FakeExtension());
    }
