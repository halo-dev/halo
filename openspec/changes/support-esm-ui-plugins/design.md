...
The bundler kit's Rsbuild integration tests and linked-workspace development use Rsbuild 2 and the matching Vue plugin so the preset and caller do not contribute incompatible major-version plugin instances. The published peer contract remains responsible for supplying the caller's supported toolchain; a linked checkout must not silently test against or inject an older auto-installed peer than the real provider project uses.

Both helpers continue owning their framework integration so existing callers compile Vue SFCs without additional setup. Callers customize that one built-in integration through a top-level `vue` option, typed as the native `@vitejs/plugin-vue` or `@rsbuild/plugin-vue` options for the selected helper. Vue compiler plugins do not belong in the nested caller plugin list because Vite and Rsbuild configuration merging can otherwise retain both framework transforms. Omitting `vue` preserves the current default plugin factory behavior, while unrelated caller plugins still merge after the provider defaults.
...
Rsbuild's development environment has a separate `dev.assetPrefix` default based on the development server base. The ESM preset sets that development prefix to `auto` as well as the production output prefix, so `rsbuild build --watch --env-mode=development` keeps the same relocatable resource contract across the initial compilation and watched rebuilds. This affects build-watch output only and does not introduce cross-Halo HMR.
...
The legacy JavaScript generated for the current descriptor includes executable code only from currently classified IIFE providers. Its `enabledPlugins` and `enabledUiPlugins` compatibility metadata includes every valid enabled provider regardless of IIFE or ESM format, so a legacy provider continues to detect another provider after that provider migrates to ESM. The existing aggregate CSS endpoint and compatibility aliases remain available for Halo 2.x, but emit ordered `@import url("<direct provider style URL>")` rules instead of concatenating provider CSS under the API URL base. This compatibility bridge carries an adjacent `TODO(Halo 3)` removal comment. Existing globals, plugin-name ordering, theme module naming, and `ui`-before-`console` resource selection remain compatible for legacy artifacts.
...
Registration uses a prepare/validate/commit boundary per provider. Existing successful route conflicts retain Vue Router's last-registration-wins behavior. Halo assigns internal identities to anonymous route parents before registering Halo-managed routes so a replaced named child can be restored if a later synchronous mutation fails. A route already registered outside this managed path under an anonymous parent does not make an otherwise compatible provider fail before mutation; successful replacement retains legacy behavior, while a later failed transaction reports the route as an incomplete rollback and uses a full page reload as recovery. The commit records removal or restoration handles for routes, store entries, components, extensions, and other registries where supported. A synchronous commit failure invokes those handles in reverse order before the next provider is registered. Existing framework registries do not make every mutation reversible, so an incomplete rollback is diagnosed and a full page reload remains the final recovery boundary.
...
- [A provider fails after replacing a named route and partially mutating host registries] → Give Halo-managed anonymous route parents an internal identity, preserve last-registration-wins on success, restore the replaced route on synchronous failure where possible, and diagnose a pre-existing unidentifiable parent as an incomplete rollback that requires reload.
