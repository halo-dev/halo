...
- Add an authenticated provider descriptor that references the existing plugin and theme static resource mappings with a version query for cache invalidation, exposes provider-owned startup styles in stable order, and drives a mixed loader that starts styles and ESM entries in parallel before preserving stable registration order.
