...
class HaloSecurityJacksonModuleTest {
    ...
    void codecHaloOAuth2AuthenticationTokenTest() throws JsonProcessingException {
        codecAssert(
                haloUser -> {
                    var oauth2User =
                            new DefaultOAuth2User(List.of(), Map.of("name", "halo"), "name");
                    var oauth2Token =
                            new OAuth2AuthenticationToken(oauth2User, List.of(), "github");
    ...
    void codecAssert(Function<HaloUser, Authentication> authenticationConverter)
        ...
        var userDetails =
                User.withUsername("faker").password("123456").authorities("ROLE_USER").build();
