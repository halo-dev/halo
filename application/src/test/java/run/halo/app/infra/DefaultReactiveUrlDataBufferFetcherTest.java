...
class DefaultReactiveUrlDataBufferFetcherTest {
    ...
    @InjectMocks DefaultReactiveUrlDataBufferFetcher fetcher;
    ...
    void fetchShouldReturnDataBuffers() {
        ...
        fetcher.setWebClient(
                WebClient.builder().exchangeFunction(request -> Mono.just(response)).build());
    ...
    void fetchShouldMapUnknownHostExceptionToServerWebInputException() {
        ...
        var error =
                new WebClientRequestException(
                        cause,
                        HttpMethod.GET,
                        URI.create("http://nonexistent-host/"),
                        HttpHeaders.EMPTY);

        fetcher.setWebClient(
                WebClient.builder().exchangeFunction(request -> Mono.error(error)).build());
