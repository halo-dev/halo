...
class DefaultExternalLinkProcessorTest {
    ...
    void process() throws MalformedURLException {
        ...
        assertThat(externalLinkProcessor.processLink("https://guqing.xyz/test")).isEqualTo("https://guqing.xyz/test");

        when(externalUrlSupplier.getRaw())
                .thenReturn(URI.create("https://halo.run/").toURL());
        assertThat(externalLinkProcessor.processLink("/test")).isEqualTo("https://halo.run/test");
        assertThat(externalLinkProcessor.processLink("https://halo.run/test")).isEqualTo("https://halo.run/test");
    }

    @ParameterizedTest
    @MethodSource("processUriTestWithoutServerWebExchangeArguments")
    void processUriWithoutServerWebExchange(String link, String expectedLink) throws MalformedURLException {
        lenient().when(externalUrlSupplier.getRaw()).thenReturn(new URL("https://www.halo.run/context-path"));
    ...
    static Stream<Arguments> processUriTestWithoutServerWebExchangeArguments() {
        return Stream.of(
                ...
                Arguments.of("/halo?query#fragment", "https://www.halo.run/context-path/halo?query#fragment"),
    ...
    void processUriWithServerWebExchange(String link, String expectLink) throws MalformedURLException {
        lenient()
                .when(externalUrlSupplier.getRaw())
    ...
    static Stream<Arguments> processUriTestWithServerWebExchangeArguments() {
        return Stream.of(
                Arguments.of("http://localhost:8090/halo?query#fragment", "http://localhost:8090/halo?query#fragment"),
                ...
                Arguments.of("/halo?query#fragment", "https://antoher.halo.run/context-path/halo?query#fragment"),
                Arguments.of("/halo/subpath", "https://antoher.halo.run/context-path/halo/subpath"),
                Arguments.of("/halo/中文", "https://antoher.halo.run/context-path/halo/%E4%B8%AD%E6%96%87"),
