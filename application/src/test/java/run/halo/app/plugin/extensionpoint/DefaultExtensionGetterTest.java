...
class DefaultExtensionGetterTest {
    ...
    void shouldGetExtensionBySingletonDefinitionWhenExtensionPointEnabledSet() {
        ...
        when(extensionPointDefinitionGetter.getByClassName(any()))
                .thenReturn(Mono.fromSupplier(() -> createExtensionPointDefinition(
                        "fake-extension-point", FakeExtensionPoint.class, ExtensionPointType.SINGLETON)));

        when(extensionDefinitionGetter.get(eq("fake-extension")))
                .thenReturn(Mono.fromSupplier(() -> createExtensionDefinition(
                        "fake-extension", FakeExtensionPointImpl.class, "fake-extension-point")));

        when(configFetcher.fetch(GROUP, ExtensionPointEnabled.class)).thenReturn(Mono.fromSupplier(() -> {
            var extensionPointEnabled = new ExtensionPointEnabled();
            extensionPointEnabled.put("fake-extension-point", new LinkedHashSet<>(List.of("fake-extension")));
            return extensionPointEnabled;
        }));

        @SuppressWarnings("unchecked")
        ObjectProvider<FakeExtensionPoint> objectProvider = mock(ObjectProvider.class);
        when(objectProvider.orderedStream()).thenReturn(Stream.of(new FakeExtensionPointDefaultImpl()));
        ...
        doReturn(List.of(extensionImpl)).when(spyGetter).lookExtensions(eq(FakeExtensionPoint.class));

        spyGetter
                .getEnabledExtensions(FakeExtensionPoint.class)
    ...
    void shouldGetDefaultSingletonDefinitionWhileExtensionPointEnabledNotSet() {
        ...
        when(configFetcher.fetch(GROUP, ExtensionPointEnabled.class)).thenReturn(Mono.empty());

        @SuppressWarnings("unchecked")
        ObjectProvider<FakeExtensionPoint> objectProvider = mock(ObjectProvider.class);
        var extensionDefaultImpl = new FakeExtensionPointDefaultImpl();
        when(objectProvider.orderedStream()).thenReturn(Stream.of(extensionDefaultImpl));
        when(beanFactory.getBeanProvider(FakeExtensionPoint.class)).thenReturn(objectProvider);

        var spyGetter = spy(getter);
        doReturn(List.of()).when(spyGetter).lookExtensions(eq(FakeExtensionPoint.class));
    ...
    void shouldGetMultiInstanceExtensionWhileExtensionPointEnabledSet() {
        ...
        when(extensionPointDefinitionGetter.getByClassName(any()))
                .thenReturn(Mono.fromSupplier(() -> createExtensionPointDefinition(
                        "fake-extension-point", FakeExtensionPoint.class, ExtensionPointType.MULTI_INSTANCE)));
        ...
        when(extensionDefinitionGetter.get(eq("default-fake-extension")))
                .thenReturn(Mono.fromSupplier(() -> createExtensionDefinition(
                        "default-fake-extension", FakeExtensionPointDefaultImpl.class, "fake-extension-point")));

        when(configFetcher.fetch(GROUP, ExtensionPointEnabled.class)).thenReturn(Mono.fromSupplier(() -> {
            var extensionPointEnabled = new ExtensionPointEnabled();
            extensionPointEnabled.put(
                    "fake-extension-point", new LinkedHashSet<>(List.of("default-fake-extension", "fake-extension")));
            return extensionPointEnabled;
        }));
        ...
        var spyGetter = spy(getter);
        doReturn(List.of(extensionImpl, anotherExtensionImpl))
                .when(spyGetter)
    ...
    void shouldGetExtensionsFromPluginManagerAndApplicationContext() {
        ...
        var spyGetter = spy(getter);
        doReturn(List.of(extensionFromPlugin)).when(spyGetter).lookExtensions(eq(FakeExtensionPoint.class));

        when(beanFactory.getBeanProvider(FakeExtensionPoint.class)).thenReturn(extensionPointObjectProvider);
        when(extensionPointObjectProvider.orderedStream()).thenReturn(Stream.of(extensionFromAppContext));
    ...
    ExtensionPointDefinition createExtensionPointDefinition(String name, Class<?> clazz, ExtensionPointType type) {
