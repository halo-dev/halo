...
class SystemConfigReconcilerTest {
    ...
    void reconcileShouldDoNothingWhenConfigMapNotFound() {
        ...
        when(client.fetch(ConfigMap.class, SystemSetting.SYSTEM_CONFIG)).thenReturn(Optional.empty());
    ...
    void reconcileShouldDoNothingWhenConfigMapIsDeleted() {
        ...
        when(client.fetch(ConfigMap.class, SystemSetting.SYSTEM_CONFIG)).thenReturn(Optional.of(systemConfigMap));
    ...
    void reconcileShouldUpdateChecksumAndPublishEventWhenDataChanges() {
        ...
        ArgumentCaptor<SystemConfigChangedEvent> eventCaptor = ArgumentCaptor.forClass(SystemConfigChangedEvent.class);
    ...
    void reconcileShouldNotUpdateWhenChecksumUnchanged() {
        ...
        var existingChecksum = com.google.common.hash.Hashing.sha256()
                .hashString(systemConfigMap.getData().toString(), java.nio.charset.StandardCharsets.UTF_8)
                .toString();
        systemConfigMap.getMetadata().getAnnotations().put(Constant.CHECKSUM_CONFIG_ANNO, existingChecksum);
    ...
    void reconcileShouldPreserveDataSnapshotForComparison() {
        ...
        systemConfigMap.getMetadata().getAnnotations().put("halo.run/data-snapshot", "{\"key1\":\"old-value\"}");
    ...
    void reconcileShouldHandleNoDefaultConfig() {
        ...
        when(client.fetch(ConfigMap.class, SystemSetting.SYSTEM_CONFIG)).thenReturn(Optional.of(systemConfigMap));
        when(client.fetch(ConfigMap.class, SystemSetting.SYSTEM_CONFIG_DEFAULT)).thenReturn(Optional.empty());
