...
class ThumbnailResourceTransformerTest {
    ...
    @Mock LocalThumbnailService localThumbnailService;

    @Mock ResourceTransformerChain transformerChain;

    @Mock Resource resource;

    @InjectMocks ThumbnailResourceTransformer thumbnailResourceTransformer;

    @Test
    void shouldNotTransformWithoutWidthQuery() {
        var exchange =
                MockServerWebExchange.builder(MockServerHttpRequest.get("/halo.png").build())
                        .build();
    ...
    void shouldNotTransformWithNonFileResource() {
        var exchange =
                MockServerWebExchange.builder(
                                MockServerHttpRequest.get("/halo.png")
    ...
    void shouldNotTransformWithUnsupportedImageType() {
        var exchange =
                MockServerWebExchange.builder(
                                MockServerHttpRequest.get("/halo.svg")
                                        .queryParam("width", "400")
                                        .build())
                        .build();
