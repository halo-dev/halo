...
class ThemeEndpointTest {
    ...
    void setUp() throws IOException {
        ...
        webTestClient =
                WebTestClient.bindToRouterFunction(themeEndpoint.endpoint()).build();
    }
    ...
    class UpgradeTest {
        ...
        void shouldNotOkIfThemeNotInstalled() {
            ...
            bodyBuilder.part("file", new FileSystemResource(defaultTheme)).contentType(MediaType.MULTIPART_FORM_DATA);

            when(themeService.upgrade(eq("invalid-missing-manifest"), isA(Publisher.class)))
                    .thenReturn(Mono.error(() -> new ServerWebInputException("Failed to upgrade theme")));
        ...
        void shouldUpgradeSuccessfullyIfThemeInstalled() {
            ...
            when(themeService.upgrade(eq("default"), isA(Publisher.class))).thenReturn(Mono.just(newTheme));
    ...
    void install() {
        ...
        when(themeService.install(any())).thenReturn(Mono.error(new RuntimeException("Fake exception")));
