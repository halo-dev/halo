...
class DefaultIndexEngineTest {
    ...
    @Mock Indices<Fake> indices;

    @Mock SingleValueIndex<Fake, String> singleValueIndex;

    @InjectMocks DefaultIndexEngine engine;
    ...
    void shouldRetrieveWithConditionsAndPage() {
        var options = ListOptions.builder().andQuery(Queries.all("metadata.name")).build();
    ...
    void shouldRetrieveAllWithConditions() {
        ...
        assertEquals(
                List.of("3", "2", "1"), StreamSupport.stream(result.spliterator(), false).toList());
    }
    ...
    void shouldRetrieveTopNWithConditions() {
        ...
        assertEquals(
                List.of("5", "4", "3"), StreamSupport.stream(result.spliterator(), false).toList());
