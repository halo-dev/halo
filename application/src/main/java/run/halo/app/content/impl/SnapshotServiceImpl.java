...
public class SnapshotServiceImpl implements SnapshotService {
                                ...
                                Mono.defer(
                                        ...
                                        .doOnNext(
                                                snapshot -> {
                                                    ...
                                                    var annotations =
                                                            snapshot.getMetadata().getAnnotations();
                                                    if (annotations == null) {
                                                    ...
                                                    annotations.put(
                                                            Snapshot.PATCHED_CONTENT_ANNO,
                                                            patchedContent);
                                                    annotations.put(
                                                            Snapshot.PATCHED_RAW_ANNO, patchedRaw);
                                                }));
    }

    @Override
    public Mono<Snapshot> patchAndCreate(
    ...
    @Override
    public Mono<Snapshot> patchAndUpdate(
            Snapshot snapshot, Snapshot baseSnapshot, Content content) {
        return Mono.just(snapshot)
                .doOnNext(s -> this.patch(s, baseSnapshot, content))
