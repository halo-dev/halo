...
public abstract class AbstractCommentService {
        ...
        return ReactiveSecurityContextHolder.getContext()
                .flatMap(
                        securityContext -> {
                            ...
                            var roles =
                                    AuthorityUtils.authoritiesToRoles(
                                            authentication.getAuthorities());
                            return roleService.contains(
                                    roles, Set.of(AuthorityUtils.COMMENT_MANAGEMENT_ROLE_NAME));
                        });
    }
    ...
    private Mono<CommentStats> fetchStats(String meterName) {
        ...
        return counterService
                .getByName(meterName)
                .map(counter -> CommentStats.builder().upvote(counter.getUpvote()).build())
