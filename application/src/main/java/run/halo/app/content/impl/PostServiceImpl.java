...
public class PostServiceImpl extends AbstractContentService implements PostService {
    ...
    private Mono<ListedPost> getListedPost(Post post) {
        ...
        var tagsMono =
                listTags(post.getSpec().getTags()).collectList().doOnNext(listedPost::setTags);
                ...
                .flatMap(
                        post -> {
                            ...
                            return draftContent(post.getSpec().getBaseSnapshot(), contentRequest)
                                    .flatMap(
                                            contentWrapper ->
                                                    waitForPostToDraftConcludingWork(
    ...
    private Mono<Post> waitForPostToDraftConcludingWork(
            String postName, ContentWrapper contentWrapper) {
        return Mono.defer(
                        () ->
                            ...
                            if (StringUtils.equals(headSnapshotName, snapshotName)) {
                                return updatePostWithRetry(
                                        ...
                                        record -> {
                                            ...
                                            record.getSpec()
                                                    .setHeadSnapshot(
                                                            record.getSpec().getReleaseSnapshot());
                                            return record;
                                        });
                            }
                            return Mono.just(post);
