...
public abstract class AbstractContentService {
        ...
        return client.fetch(Snapshot.class, baseSnapshotName)
                ...
                .flatMap(
                        baseSnapshot -> {
                            if (StringUtils.equals(snapshotName, baseSnapshotName)) {
                                var contentWrapper =
                                        ContentWrapper.patchSnapshot(baseSnapshot, baseSnapshot);
                                return Mono.just(contentWrapper);
                            }
                            return client.fetch(Snapshot.class, snapshotName)
                                    .map(
                                            snapshot ->
                                                    ContentWrapper.patchSnapshot(
                ...
                .switchIfEmpty(
                        Mono.defer(
                                () -> {
                                    log.error(
                                            "The content snapshot [{}] or base snapshot [{}] not"
                                                    + " found.",
                                            snapshotName,
                                            baseSnapshotName);
                                    return Mono.empty();
    ...
    protected Mono<ContentWrapper> draftContent(
            @Nullable String baseSnapshotName,
    ...
    private Mono<Snapshot> create(
        ...
        return client.fetch(Snapshot.class, baseSnapshotName)
                ...
                .map(
                        baseSnapshot ->
                                determineRawAndContentPatch(snapshot, baseSnapshot, contentRequest))
                .flatMap(
                        source ->
                                getContextUsername()
                                        .doOnNext(
    ...
    protected Mono<ContentWrapper> updateContent(
            String baseSnapshotName, ContentRequest contentRequest) {
                                ...
                                client.fetch(Snapshot.class, contentRequest.headSnapshotName())
                                        .flatMap(
                                                headSnapshot -> {
                                                    ...
                                                    var version =
                                                            headSnapshot.getMetadata().getVersion();
