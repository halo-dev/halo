...
class UserScopedPatHandlerImpl implements UserScopedPatHandler {
    ...
    public UserScopedPatHandlerImpl(ReactiveExtensionClient client, PatService patService) {
        this.client = client;
    ...
    public Mono<ServerResponse> create(ServerRequest request) {
        return ReactiveSecurityContextHolder.getContext()
                ...
                .flatMap(pat -> patService
                        .generateToken(pat)
                        .doOnNext(token -> {
                            if (pat.getMetadata().getAnnotations() == null) {
                                pat.getMetadata().setAnnotations(new HashMap<>());
                            }
                            pat.getMetadata().getAnnotations().put(ACCESS_TOKEN_ANNO_NAME, token);
                        })
                        .thenReturn(pat))
    ...
    public Mono<ServerResponse> list(ServerRequest request) {
        return ReactiveSecurityContextHolder.getContext()
                ...
                .flatMap(auth -> {
                    ...
                    var pats = client.list(PersonalAccessToken.class, predicate, compareCreationTimestamp(false));
