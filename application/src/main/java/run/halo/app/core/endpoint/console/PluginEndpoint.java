...
public class PluginEndpoint implements CustomEndpoint, InitializingBean {
    ...
    public RouterFunction<ServerResponse> endpoint() {
        ...
        return SpringdocRouteBuilder.route()
                .POST(
                        ...
                        this::install,
                        builder -> builder.operationId("InstallPlugin")
                        ...
                        builder -> builder.operationId("InstallPluginFromUri")
                                ...
                                .requestBody(requestBodyBuilder()
                                        ...
                                        .content(contentBuilder()
                                                ...
                                                .schema(schemaBuilder().implementation(InstallFromUriRequest.class))))
                                .response(responseBuilder().implementation(Plugin.class)))
                .POST(
                        "plugins/{name}/upgrade-from-uri",
                        this::upgradeFromUri,
                        builder -> builder.operationId("UpgradePluginFromUri")
                                ...
                                .requestBody(requestBodyBuilder()
                                        ...
                                        .content(contentBuilder()
                                                .mediaType(MediaType.APPLICATION_JSON_VALUE)
                                                .schema(schemaBuilder().implementation(UpgradeFromUriRequest.class))))
                                .response(responseBuilder().implementation(Plugin.class)))
                .POST(
                        "plugins/{name}/upgrade",
                        contentType(MediaType.MULTIPART_FORM_DATA),
                        this::upgrade,
                        builder -> builder.operationId("UpgradePlugin")
                ...
                .PUT(
                        ...
                        builder -> builder.operationId("ResetPluginConfig")
                                ...
                                .parameter(parameterBuilder()
                                        ...
                                        .required(true)
                                        .implementation(String.class))
                                .response(responseBuilder().implementation(ConfigMap.class)))
                .PUT(
                        "plugins/{name}/reload",
                        this::reload,
                ...
                .PUT(
                        ...
                        builder -> builder.operationId("ChangePluginRunningState")
                                ...
                                .requestBody(requestBodyBuilder()
                                        ...
                                        .content(contentBuilder()
                                                .mediaType(MediaType.APPLICATION_JSON_VALUE)
                                                .schema(schemaBuilder().implementation(RunningStateRequest.class))))
                                .response(responseBuilder().implementation(Plugin.class)))
                ...
                .GET(
                        "plugins/{name}/setting",
                        this::fetchPluginSetting,
                    ...
                    if (useLastModified) {
                        try {
                            ...
                            if (e instanceof FileNotFoundException) {
                                return Mono.error(new NoResourceFoundException(request.uri(), "bundle.js"));
                            }
                            return Mono.error(e);
                        }
                    }
                ...
                .map(s -> pluginService.getCssBundle(s).flatMap(cssRes -> {
                    ...
                    if (useLastModified) {
                        try {
                            ...
                            if (e instanceof FileNotFoundException) {
                                return Mono.error(new NoResourceFoundException(request.uri(), "bundle.css"));
                            }
                            return Mono.error(e);
                        }
                    }
    ...
    URI buildJsBundleUri(String type, String version) {
        return URI.create("/apis/api.console.halo.run/v1alpha1/plugins/-/bundle." + type + "?v=" + version);
    }
    ...
    private Mono<ServerResponse> upgradeFromUri(ServerRequest request) {
        ...
        return Mono.usingWhen(
                        writeToTempFile(content), path -> pluginService.upgrade(name, path), this::deleteFileIfExists)
    ...
    private Mono<ServerResponse> install(ServerRequest request) {
        return request.multipartData()
                ...
                .flatMap(installRequest -> installRequest.getSource().flatMap(source -> {
                    if (InstallSource.FILE.equals(source)) {
                        return installFromFile(installRequest.getFile(), pluginService::install);
                    }
                    return Mono.error(new UnsupportedOperationException("Unsupported install source " + source));
                }))
                ...
                .flatMap(installRequest -> installRequest.getSource().flatMap(source -> {
                    if (InstallSource.FILE.equals(source)) {
                        return installFromFile(
                                installRequest.getFile(), path -> pluginService.upgrade(pluginName, path));
                    }
                    return Mono.error(new UnsupportedOperationException("Unsupported install source " + source));
                }))
    ...
    private Mono<Plugin> installFromFile(FilePart filePart, Function<Path, Mono<Plugin>> resourceClosure) {
        return Mono.usingWhen(writeToTempFile(filePart.content()), resourceClosure, this::deleteFileIfExists);
    }
        ...
        public ListOptions toListOptions() {
            ...
            Optional.ofNullable(queryParams.getFirst("keyword"))
                    ...
                    .ifPresent(keyword -> builder.andQuery(
                            or(contains("spec.displayName", keyword), contains("spec.description", keyword))));
    ...
    Mono<ServerResponse> list(ServerRequest request) {
        return Mono.just(request)
                .map(ListRequest::new)
                .flatMap(listRequest ->
                        client.listBy(Plugin.class, listRequest.toListOptions(), listRequest.toPageRequest()))
    ...
    public static class InstallRequest {
        ...
        public Mono<InstallSource> getSource() {
            ...
            if (!(part instanceof FormFieldPart sourcePart)) {
                return Mono.error(new ServerWebInputException("Invalid format of source field, string required."));
            }
