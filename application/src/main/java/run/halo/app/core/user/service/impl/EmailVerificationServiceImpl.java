...
public class EmailVerificationServiceImpl implements EmailVerificationService {
                                ...
                                client.get(User.class, username)
                                        .flatMap(
                                                user -> {
                                                    ...
                                                    var oldEmailToVerify =
                                                            annotations.get(User.EMAIL_TO_VERIFY);
                                                    var unsubMono =
                                                            unSubscribeVerificationEmailNotification(
                                                                    oldEmailToVerify);
                                                    var updateUserAnnoMono =
                                                            Mono.defer(
                                                                    () -> {
                                                                        annotations.put(
                                                                                User
                                                    ...
                                                    emailVerificationManager.removeCode(
                                                            username, oldEmailToVerify);
    ...
    Mono<Void> sendVerificationNotification(String username, String email) {
        ...
        if (log.isDebugEnabled()) {
            log.debug(
                    "Generated verification code for user '{}' and email '{}': {}",
                    username,
                    email,
                    code);
        }
    ...
    static class EmailVerificationManager {
        ...
        private final Cache<UsernameEmail, Boolean> blackListCache =
                CacheBuilder.newBuilder()
        ...
        boolean contains(String username, String email) {
            return emailVerificationCodeCache.getIfPresent(new UsernameEmail(username, email))
                    != null;
        }
