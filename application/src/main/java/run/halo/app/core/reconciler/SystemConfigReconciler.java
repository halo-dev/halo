...
class SystemConfigReconciler implements Reconciler<Reconciler.Request> {
    ...
    public Controller setupWith(ControllerBuilder builder) {
        ExtensionMatcher matcher =
                extension ->
                        Objects.equals(
                                extension.getMetadata().getName(), SystemSetting.SYSTEM_CONFIG);
    ...
    private SystemConfigChangedEvent computeChangedEvent(
            ConfigMap configMap, @Nullable Map<String, String> oldData) {
        return client.fetch(ConfigMap.class, SystemSetting.SYSTEM_CONFIG_DEFAULT)
                .map(
                        defaultConfigMap -> {
                            var defaultData =
                                    requireNonNullElse(
                                            defaultConfigMap.getData(), Map.<String, String>of());
                            try {
                                var mergedOldData =
                                        mergeMap(
                                ...
                                var mergedNewData =
                                        mergeMap(
                                                defaultData,
                                                requireNonNullElse(configMap.getData(), Map.of()));
                                return new SystemConfigChangedEvent(
                                        this, mergedOldData, mergedNewData);
