...
class DefaultAttachmentService implements AttachmentService {
                ...
                WebClient.builder()
                        .clientConnector(
                                new ReactorClientHttpConnector(
                                        HttpSecurityUtils.secureHttpClient()))
    ...
    public Mono<Attachment> upload(
        ...
        var getPolicyAndConfigMap =
                client.get(Policy.class, policyName)
                                ...
                                Mono.error(
                                        () ->
                                                new ServerWebInputException(
                                                        "ConfigMap name not found in Policy "
                                                                + policyName)))
                        ...
                        Mono.error(
                                () ->
                                        new ServerErrorException(
                                                "No suitable handler found for uploading the"
                                                        + " attachment.",
                                                null)))
                .doOnNext(
                        attachment -> {
    ...
    public Mono<Attachment> upload(
        ...
        return authenticationConsumer(
                authentication ->
                        upload(authentication.getName(), policyName, groupName, file, null));
    }
    ...
    public Mono<Attachment> delete(Attachment attachment) {
        ...
        return client.get(Policy.class, spec.getPolicyName())
                .flatMap(
                        policy ->
                                client.get(ConfigMap.class, policy.getSpec().getConfigMapName())
                                        .map(
                                                configMap ->
                                                        new DeleteOption(
                                                                attachment, policy, configMap)))
                .flatMap(
                        deleteOption ->
                                extensionGetter
                                        .getExtensions(AttachmentHandler.class)
                        ...
                        uri ->
                                webClient
                                        ...
                                        .onErrorMap(
                                                ...
                                                e -> {
                                                    if (e.getCause()
                                                            instanceof UnknownHostException ex) {
                                        ...
                                        .onErrorMap(
                                                ...
                                                e -> {
                                                    if (e.getCause()
                                                            instanceof DataBufferLimitException) {
                                                        return new ServerWebInputException(
                                                                """
