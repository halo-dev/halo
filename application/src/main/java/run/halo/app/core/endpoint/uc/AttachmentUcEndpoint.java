...
public class AttachmentUcEndpoint implements CustomEndpoint {
    ...
    public RouterFunction<ServerResponse> endpoint() {
        ...
        return route().POST(
                        ...
                        builder -> builder.operationId("CreateAttachmentForPost")
                                .tag(tag)
                                .description("""
                ...
                .POST(
                        ...
                        builder -> builder.operationId("ExternalTransferAttachment_1")
                                .description("""
    ...
    private Mono<ServerResponse> listMyAttachments(ServerRequest request) {
        return getCurrentUser().flatMap(username -> {
    ...
    private Mono<Attachment> waitForPermalink(Mono<Attachment> createdAttachment) {
        createdAttachment = createdAttachment.flatMap(attachment -> attachmentService
    ...
    private Mono<Void> checkPostOwnership(Mono<PostAttachmentRequest> postAttachmentRequest) {
        // check the post
        var postNotFoundError = Mono.<Post>error(() -> new NotFoundException("The post was not found or deleted."));
