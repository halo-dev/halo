...
class PatServiceImpl implements PatService {
    ...
    private Mono<PersonalAccessToken> create(
            PersonalAccessToken patRequest,
            String username,
                        ...
                        Mono.error(
                                () ->
                                        new ServerWebInputException(
                                                "The token has been revoked before.")))
                .doOnNext(
                        pat -> {
                        ...
                        Mono.error(
                                () ->
                                        new NotFoundException(
                                                "The personal access token was not found or"
                                                        + " deleted.")));
    }
                        ...
                        contextView -> {
                            var externalUrl =
                                    ServerWebExchangeContextFilter.getExchange(contextView)
                                            .map(
                                                    exchange ->
                                                            this.externalUrl.getURL(
                                                                    exchange.getRequest()))
                                            .orElse(null);
                            if (externalUrl == null) {
                                return Mono.error(
                                        new ServerWebInputException(
                                                "Server web exchange is " + "required"));
                            }
                            var claimsBuilder =
                            ...
                            var headerBuilder =
                                    JwsHeader.with(SignatureAlgorithm.RS256).keyId(this.keyId);
                            var jwt =
                                    jwtEncoder.encode(
                                            JwtEncoderParameters.from(
                                                    headerBuilder.build(), claimsBuilder.build()));
