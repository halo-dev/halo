...
class LocalAttachmentUploadHandler implements AttachmentHandler {
                    ...
                    return validateFile(file, setting)
                            .then(Mono.fromRunnable(() -> {
                                    ...
                                    .map(path -> {
                                        ...
                                        var relativePath =
                                                attachmentsRoot.relativize(path).toString();
                                        ...
                                        spec.setMediaType(Optional.ofNullable(
    ...
    private Mono<Void> validateFile(FilePart file, PolicySetting setting) {
        ...
        if (!CollectionUtils.isEmpty(setting.getAllowedFileTypes())) {
            var typeValidator = file.content().next().handle((dataBuffer, sink) -> {
                ...
                var isAllow = setting.getAllowedFileTypes().stream()
    ...
    private static void handleFileTypeError(SynchronousSink<Object> sink, String detailCode, String mimeType) {
        sink.error(new FileTypeNotAllowedException(
                "File type is not allowed", "problemDetail.attachment.upload." + detailCode, new Object[] {mimeType}));
    }
    ...
    /**
     * Write content into file. We will detect duplicate filename and auto-rename it with 3 times retry.
     *
     * @param content is file content
     * @param targetPath is target path
     * @return file path
     */
    private Mono<Path> writeContent(Flux<DataBuffer> content, Path targetPath, boolean renameIfExists) {
        return Mono.defer(() -> {
