...
public class UcPostEndpoint implements CustomEndpoint {
                        ...
                        () ->
                                route().GET(
                                        ...
                                        .POST(
                                                this::createMyPost,
                                                builder ->
                                                        builder.operationId("CreateMyPost")
                                                                .tag(tag)
                                                                .description(
                                                                        """
                                        ...
                                        .GET(
                                                ...
                                                this::getMyPost,
                                                builder ->
                                                        builder.operationId("GetMyPost")
                                                                .tag(tag)
                                                                .parameter(namePathParam)
                                                                .description(
                                                                        "Get post that belongs to"
                                                                            + " the current user.")
                                                                .response(
                                                                        responseBuilder()
                                                                                .implementation(
                ...
                getMyPost(name)
                        .flatMap(
                                post -> {
                                    ...
                                    if (patched) {
                                        return snapshotService.getPatchedBy(
                                                headSnapshotName, baseSnapshotName);
                                    }
    ...
    private Mono<ServerResponse> unpublishMyPost(ServerRequest request) {
        var name = request.pathVariable("name");
        var postMono =
                getCurrentUser().flatMap(username -> postService.getByUsername(name, username));
    ...
    private Mono<ServerResponse> updateMyPostDraft(ServerRequest request) {
                        ...
                        .filter(
                                metadata -> {
                                    ...
                                    return annotations != null
                                            && annotations.containsKey(CONTENT_JSON_ANNO);
                                })
                        .map(
                                metadata -> {
                                    var contentJson =
                                            metadata.getAnnotations().remove(CONTENT_JSON_ANNO);
                                                        ...
                                                        snapshot -> {
                                                            var snapshotName =
                                                                    snapshot.getMetadata()
                                                                            .getName();
                                                            var headSnapshotName =
                                                                    post.getSpec()
                                                                            .getHeadSnapshot();
                                                            return Objects.equals(
                                                                    snapshotName, headSnapshotName);
                                                        })
                                                .switchIfEmpty(
                                                        Mono.error(
                                                                () ->
        ...
        var setContributor =
                getCurrentUser()
                        .flatMap(
                                username ->
                                        snapshotMono.doOnNext(
                                                snapshot ->
                                                        Snapshot.addContributor(
                                                                snapshot, username)));

        var getBaseSnapshot =
                postMono.map(post -> post.getSpec().getBaseSnapshot())
                        .flatMap(snapshotService::getBy);

        var updatedSnapshot =
                getBaseSnapshot.flatMap(
                        baseSnapshot ->
                                contentMono.flatMap(
                                        content ->
                                                postMono.flatMap(
        ...
        var createdPost =
                getCurrentUser()
                        ...
                        .map(
                                post ->
                                        new PostRequest(
                                                post, ContentUpdateParam.from(getContent(post))))
