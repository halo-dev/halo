...
public class DefaultNotificationReasonEmitter implements NotificationReasonEmitter {
    ...
    @Override
    public Mono<Void> emit(String reasonType, Consumer<ReasonPayload.ReasonPayloadBuilder> builder) {
    ...
    Mono<Void> validateReason(Reason reason) {
        ...
        return client.fetch(ReasonType.class, reasonTypeName)
                ...
                .doOnNext(reasonType -> {
                    ...
                    nullSafeList(reasonType.getSpec().getProperties()).forEach(property -> {
