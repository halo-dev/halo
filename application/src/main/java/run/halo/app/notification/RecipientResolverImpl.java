...
public class RecipientResolverImpl implements RecipientResolver {
                ...
                .filter(subscription -> {
                    ...
                    if (hasSubject(interestReason)) {
                        ...
                        return expressionMatch(
                                subscription.getMetadata().getName(), interestReason.getExpression(), reason);
                    }
                    return false;
                })
                .map(subscription -> {
                    var id = UserIdentity.of(
    ...
    boolean expressionMatch(String subscriptionName, String expressionStr, Reason reason) {
        try {
            ...
            var result = expression.getValue(evaluationContext, exprRootObject(reason), Boolean.class);
    ...
    EvaluationContext createEvaluationContext() {
        return SimpleEvaluationContext.forPropertyAccessors(
                        DataBindingPropertyAccessor.forReadOnlyAccess(), new MapAccessor(), new JsonPropertyAccessor())
