...
public class UserNotificationPreferencesEndpoint implements CustomEndpoint {
        ...
        return () ->
                SpringdocRouteBuilder.route()
                        .GET(
                                ...
                                this::listNotificationPreferences,
                                builder ->
                                        builder.operationId("ListUserNotificationPreferences")
                                                .description(
                                                        "List notification preferences for the"
                                                                + " authenticated user.")
                                                .tag(tag)
                                                .parameter(
                                                        parameterBuilder()
                        ...
                        .POST(
                                ...
                                builder ->
                                        builder.operationId("SaveUserNotificationPreferences")
                                                .description(
                                                        "Save notification preferences for the"
                                                                + " authenticated user.")
                                                .tag(tag)
                                                .parameter(
                                                        parameterBuilder()
                                                        ...
                                                        reasonTypeNotifierRequest -> {
                                                            ...
                                                            notifierSetting.setNotifiers(
                                                                    notifiers == null
                                                                            ...
                                                                            : Set.copyOf(
                                                                                    notifiers));
                                                            reasonTypeNotifierMap.put(
                                                                    reasonType, notifierSetting);
                                                        });
                                                return userNotificationPreferenceService.saveByUser(
                                                        username, preference);
                                            });
                        })
                .then(
                        Mono.defer(
                                () ->
                                        listReasonTypeNotifierMatrix(username)
                                                .flatMap(
                                                        result ->
                                                                ServerResponse.ok()
                                                                        .bodyValue(result))));
    ...
    private static <T> Map<String, Integer> toNameIndexMap(
            List<T> collection, Function<T, String> nameGetter) {
    ...
    Mono<ReasonTypeNotifierMatrix> listReasonTypeNotifierMatrix(String username) {
        ...
        return client.listAll(ReasonType.class, listOptions, ExtensionUtil.defaultSort())
                ...
                .flatMap(
                        reasonTypes ->
                                client.list(
                                                NotifierDescriptor.class,
                                                null,
                                                Comparators.defaultComparator())
                                        .map(
                                                notifier ->
                                                        new NotifierInfo(
                                                                notifier.getMetadata().getName(),
                ...
                .flatMap(
                        tuple2 -> {
                            ...
                            var reasonTypeIndexMap =
                                    toNameIndexMap(reasonTypes, ReasonTypeInfo::name);
                            var notifierIndexMap =
                                    toNameIndexMap(matrix.getNotifiers(), NotifierInfo::name);
                                    ...
                                    .doOnNext(
                                            preference -> {
                                                ...
                                                for (ReasonTypeInfo reasonType : reasonTypes) {
                                                    var reasonTypeIndex =
                                                            reasonTypeIndexMap.get(
                                                                    reasonType.name());
                                                    var notifierNames =
                                                            reasonTypeNotifierMap.getNotifiers(
                                                                    reasonType.name());
                                                    for (String notifierName : notifierNames) {
                                                        // Skip if the notifier enabled in the user
                                                        // preference does not
                                                        // exist to avoid null index
                                                        if (!notifierIndexMap.containsKey(
                                                                notifierName)) {
                                                            continue;
                                                        }
                                                        var notifierIndex =
                                                                notifierIndexMap.get(notifierName);
                                                        stateMatrix[reasonTypeIndex][
                                                                        notifierIndex] =
                                                                true;
                                                    }
                                                }
    ...
    record ReasonTypeInfo(
            String name, String displayName, String description, Set<String> uiPermissions) {

        public static ReasonTypeInfo from(ReasonType reasonType) {
            var uiPermissions =
                    Optional.of(MetadataUtil.nullSafeAnnotations(reasonType))
