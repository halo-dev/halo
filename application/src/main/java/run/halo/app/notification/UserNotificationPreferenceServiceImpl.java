...
public class UserNotificationPreferenceServiceImpl implements UserNotificationPreferenceService {
    ...
    @Override
    public Mono<Void> saveByUser(String username, UserNotificationPreference userNotificationPreference) {
        var configName = buildUserPreferenceConfigMapName(username);
        return client.fetch(ConfigMap.class, configName)
                ...
                .flatMap(config -> {
                    ...
                    config.getData().put(NOTIFICATION_PREFERENCE, JsonUtils.objectToJson(userNotificationPreference));
