 ...
 *
 * <p>All kinds of {@link Autowired} are supported (see example below). If no {@link ApplicationContext} is available
 * (this is the case if either the related plugin is not a {@link BasePlugin} or the given plugin manager is not a
 * {@link HaloPluginManager}), standard Java reflection will be used to instantiate an extension.
 *
 * <p>Creates a new extension instance every time a request is done. Example of supported autowire modes:
 *
 * <pre>{@code
 * @Extension
    ...
    protected <T> T createWithoutSpring(final Class<T> extensionClass) throws IllegalArgumentException {
        final Constructor<?> constructor = getPublicConstructorWithShortestParameterList(extensionClass)
                ...
                .orElseThrow(() -> new IllegalArgumentException(
                        "Extension class '" + nameOf(extensionClass) + "' must have at least one public constructor."));
        try {
