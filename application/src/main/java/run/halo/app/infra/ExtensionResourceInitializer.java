    ...
    public void start() {
        ...
        Flux.fromIterable(locations)
                .doOnNext(
                        location ->
                                log.debug(
                                        "Trying to initialize extension resources from location:"
                                                + " {}",
                                        location))
                ...
                .doOnNext(
                        resource ->
                                log.debug(
                                        "Initializing extension resource from location: {}",
                                        resource))
                ...
                .flatMap(
                        existingExt -> {
                            if (ExtensionUtil.hasDoNotOverwriteLabel(existingExt)) {
                                log.debug(
                                        "Extension {} is marked as do-not-overwrite, skipping"
                                                + " update",
                            ...
                            extension
                                    .getMetadata()
                                    .setVersion(existingExt.getMetadata().getVersion());
