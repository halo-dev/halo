...
class DefaultSystemConfigFetcher
    ...
    private final Mono<Map<String, String>> configMapMono =
            Mono.defer(
                            () -> {
                                ...
                                return computeSystemConfig()
                                        .mapNotNull(
                                                configMap -> {
                                                    if (configMapCache.compareAndSet(
                                                            null, configMap.getData())) {
    ...
    private Mono<ConfigMap> computeSystemConfig() {
        ...
        return Mono.zip(
                getDefaultConfigMap,
                getOverrideConfigMap,
                (defaultConfigMap, overrideConfigMap) -> {
                    try {
                        return SystemConfigUtils.mergeConfigMap(
                                defaultConfigMap, overrideConfigMap);
