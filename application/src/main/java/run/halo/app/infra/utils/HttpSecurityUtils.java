...
public enum HttpSecurityUtils {
    ...
    /**
     * Create a secure HttpClient that blocks connections to private, local, and special-use IP addresses to prevent
     * SSRF attacks.
     *
                            ...
                            return body.handle((buffer, sink) -> {
                                ...
                                if (remainder < 0) {
                                    ...
                                    sink.error(new DataBufferLimitException(
                                            "Response body exceeds the maximum allowed size of " + maxByteCount
                                                    + " bytes"));
                                } else {
                                    sink.next(buffer);
                                }
