...
public class HaloErrorWebExceptionHandler extends DefaultErrorWebExceptionHandler {
                ...
                .flatMap(
                        themeContext ->
                                super.renderErrorView(request)
                                        .contextWrite(
                                                Context.of(ThemeContext.class, themeContext)));
    }
    ...
    protected Mono<ServerResponse> renderErrorView(
        ...
        return Mono.deferContextual(
                contextView -> {
                    Optional<ThemeContext> themeContext =
                            contextView.getOrEmpty(ThemeContext.class);
                    if (themeContext.isPresent()
                            && templateAvailabilityProvider.isTemplateAvailable(
                                    themeContext.get(), viewName)) {
