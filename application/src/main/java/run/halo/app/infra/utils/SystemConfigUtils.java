...
public enum SystemConfigUtils {
    ...
    public static ConfigMap mergeConfigMap(ConfigMap defaultConfigMap, ConfigMap overrideConfigMap)
        ...
        var mergedData =
                mergeMap(
                        ...
                        overrideConfigMap.getData() != null
                                ? overrideConfigMap.getData()
                                : Map.of());
    ...
    public static boolean populateChecksum(ConfigMap configMap) {
        var toHash = Optional.ofNullable(configMap.getData()).map(Objects::toString).orElse("");
        var checksum = Hashing.sha256().hashString(toHash, StandardCharsets.UTF_8).toString();
                ...
                .map(
                        dataJson ->
                                JsonUtils.jsonToObject(
                                        dataJson, new TypeReference<Map<String, String>>() {}))
                .orElse(Map.of());
    }
}
