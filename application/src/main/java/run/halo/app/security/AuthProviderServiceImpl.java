...
public class AuthProviderServiceImpl implements AuthProviderService {
    ...
    public Mono<AuthProvider> enable(String name) {
        return client.get(AuthProvider.class, name)
                .flatMap(
                        authProvider ->
                                updateAuthProviderEnabled(name, true).thenReturn(authProvider));
    }
    ...
    public Mono<AuthProvider> disable(String name) {
        return client.get(AuthProvider.class, name)
                ...
                .flatMap(
                        authProvider ->
                                updateAuthProviderEnabled(name, false).thenReturn(authProvider));
    }

    @Override
    public Mono<List<ListedAuthProvider>> listAll() {
        var listOptions = ListOptions.builder().andQuery(ExtensionUtil.notDeleting()).build();
        var allProvidersMono =
                client.listAll(AuthProvider.class, listOptions, ExtensionUtil.defaultSort())
                        .map(this::convertTo)
                        .collectList();

        var boundProvidersMono =
                listMyConnections()
                        ...
                        tuple3 -> {
                            ...
                            var stateMap =
                                    tuple3.getT3().stream()
                                            .collect(
                                                    Collectors.toMap(
                                                            SystemSetting.AuthProviderState
                                                                    ::getName,
                                                            Function.identity()));
                            return allProviders.stream()
                                    ...
                                    .sorted(
                                            Comparator.comparingInt(ListedAuthProvider::getPriority)
                ...
                .flatMapMany(
                        states -> {
                            ...
                            return client.listAll(
                                    ...
                                    .map(
                                            provider ->
                                                    new AuthProviderWithPriority()
                                                            .setAuthProvider(provider)
                                                            .setPriority(
                                                                    namePriorityMap.getOrDefault(
                        ...
                        username -> {
                            var listOptions =
                                    ListOptions.builder()
                                            ...
                                            .build();
                            return client.listAll(
                                    UserConnection.class, listOptions, ExtensionUtil.defaultSort());
                        });
    }
                        ...
                        () ->
                                getSystemConfigMap()
                                        .flatMap(
                                                configMap -> {
                                                    var providerConfig =
                                                            getAuthProviderConfig(configMap);
                                                    var stateToFoundOpt =
                                                            providerConfig.getStates().stream()
                                                                    .filter(
                                                                            state ->
                                                                                    state.getName()
                                                                                            .equals(
