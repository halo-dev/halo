...
class TokenBasedRememberMeServices implements ServerLogoutHandler, RememberMeServices {
    ...
    protected Mono<UserDetails> processAutoLoginCookie(
            String[] cookieTokens, ServerWebExchange exchange) {
                ...
                .switchIfEmpty(
                        Mono.error(
                                new UsernameNotFoundException(
                                        "User '" + cookieTokens[0] + "' not found")))
                .flatMap(
                        userDetails -> {
                            // Check signature of token matches remaining details. Must do this
                            // after user
                            // lookup, as we need the DAO-derived password. If efficiency was a
                            // major issue,
                            // just add in a UserCache implementation, but recall that this method
                            // is usually
                            // only called once per HttpSession - if the token is valid, it will
                            // cause
                            // SecurityContextHolder population, whilst if invalid, will cause the
                            // cookie to
                            // be cancelled.
                            String actualTokenSignature;
                            String actualAlgorithm = DEFAULT_ALGORITHM;
                            // If the cookie value contains the algorithm, we use that algorithm to
                            // check the
                            // signature
    ...
    protected Mono<Authentication> createSuccessfulAuthentication(
            ServerWebExchange exchange, UserDetails user) {
        return getKey().map(
                        key ->
    ...
    protected Mono<Void> onLoginSuccess(
            ServerWebExchange exchange, Authentication successfulAuthentication) {
        return Mono.defer(() -> retrieveUsernamePassword(successfulAuthentication))
                .flatMap(
                        pair -> {
                            ...
                            var expiryTimeMs =
                                    calculateExpireTime(exchange, successfulAuthentication);
                            return makeTokenSignature(
                                            expiryTimeMs, username, password, DEFAULT_ALGORITHM)
                                    .doOnNext(
                                            signatureValue -> {
                                                setCookie(
                                                        new String[] {
                                                            username,
                                                            Long.toString(expiryTimeMs),
                                                            DEFAULT_ALGORITHM,
                                                            signatureValue
                                                        },
                                                        exchange);
                                                if (log.isDebugEnabled()) {
                                                    log.debug(
                                                            "Added remember-me cookie for user"
                                                                    + " '{}', expiry: '{}'",
                                                            username,
    ...
    private Mono<Boolean> rememberMeRequested(ServerWebExchange exchange) {
        ...
        return Mono.justOrEmpty(exchange.getRequest().getQueryParams().getFirst(parameterName))
                ...
                .switchIfEmpty(
                        Mono.defer(
                                () -> parameterRequestCache.getParameter(exchange, parameterName)))
                                ...
                                .flatMap(
                                        user -> {
                                            ...
                                            if (!StringUtils.hasLength(existingPassword)) {
                                                log.debug(
                                                        "Unable to obtain password for user: {}",
                                                        username);
                                                return Mono.empty();
                                            }
                                            return Mono.just(
                ...
                .onErrorMap(
                        ...
                        ex ->
                                new IllegalStateException(
                                        "No " + algorithm + " algorithm available!", ex));
    }
