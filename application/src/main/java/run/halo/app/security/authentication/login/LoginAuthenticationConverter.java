...
public class LoginAuthenticationConverter extends ServerFormLoginAuthenticationConverter {
    ...
    public LoginAuthenticationConverter(
            CryptoService cryptoService, RateLimiterRegistry rateLimiterRegistry) {
                ...
                .<Authentication>flatMap(
                        token -> {
                            if (token.getCredentials() == null) {
                                return Mono.error(
                                        new BadCredentialsException("Empty credentials."));
                            }
                            ...
                            try {
                                ...
                                return Mono.error(
                                        new BadCredentialsException("Invalid Base64 scheme."));
                            }
    ...
    private <T> RateLimiterOperator<T> createIpBasedRateLimiter(ServerWebExchange exchange) {
        ...
        var rateLimiter =
                rateLimiterRegistry.rateLimiter(
                        "authentication-from-ip-" + clientIp, "authentication");
