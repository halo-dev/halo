...
public class InMemoryReactiveIndexedSessionRepository extends ReactiveMapSessionRepository
    ...
    private final ConcurrentMap<String, Set<IndexKey>> sessionIdIndexMap = new ConcurrentHashMap<>();
    private final ConcurrentMap<IndexKey, Set<String>> indexSessionIdMap = new ConcurrentHashMap<>();

    /**
     * Prevent other requests from being parsed and acquiring the session during its deletion, which could result in an
     * unintended renewal. Currently, it acts as a buffer, and having a slightly prolonged expiration period is
     * sufficient.
     */
    ...
    public Mono<Void> save(MapSession session) {
        ...
        return super.save(session).then(updateIndex(session));
    }

    @Override
    public Mono<Void> deleteById(String id) {
        return removeIndex(id).then(Mono.defer(() -> {
    ...
    Mono<Void> removeIndex(String sessionId) {
        return getIndexes(sessionId)
                .doOnNext(indexKey -> indexSessionIdMap.computeIfPresent(indexKey, (key, sessionIdSet) -> {
    ...
    Mono<Void> updateIndex(MapSession session) {
        return removeIndex(session.getId())
                ...
                .then(Mono.defer(() -> {
                    indexResolver.resolveIndexesFor(session).forEach((name, value) -> {
                        IndexKey indexKey = new IndexKey(name, value);
                        indexSessionIdMap
                                .computeIfAbsent(indexKey, unusedSet -> ConcurrentHashMap.newKeySet())
                                .add(session.getId());
                        // Update sessionIdIndexMap
                        sessionIdIndexMap
                                .computeIfAbsent(session.getId(), unusedSet -> ConcurrentHashMap.newKeySet())
                                .add(indexKey);
                    });
                    return Mono.empty();
                }))
