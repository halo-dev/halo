...
class DeviceServiceImpl implements DeviceService {
    ...
    public Mono<Void> loginSuccess(ServerWebExchange exchange, Authentication authentication) {
        return updateExistingDevice(exchange, authentication)
                .switchIfEmpty(
                        createDevice(exchange, authentication)
                                ...
                                .doOnNext(
                                        device -> {
                                            ...
                                            eventPublisher.publishEvent(
                                                    new NewDeviceLoginEvent(this, device));
                                        }))
                .then();
    }
                                                        ...
                                                        .flatMap(
                                                                session -> {
                                                                    ...
                                                                    Optional.ofNullable(
                                                                                    exchange
                                                                                            .getAttribute(
                                                                                                    REMEMBER_ME_SERIES_REQUEST_NAME))
                                                                            .filter(
                                                                                    String.class
                                                                                            ::isInstance)
    ...
    private Mono<Device> updateWithRetry(
        ...
        return Mono.defer(
                        () ->
                                client.fetch(Device.class, deviceId)
                                        .flatMap(
                                                device -> {
                                                    if (!Objects.equals(
                                                            username,
                                                        ...
                                                        log.debug(
                                                                "Principal name mismatch for device"
                                                                    + " {}, expected {}, actual {},"
                                                                    + " revoking device",
                                                                deviceId,
                                                                username,
    ...
    private Mono<Device> updateExistingDevice(
        ...
        return updateWithRetry(
                ...
                existingDevice -> {
                    ...
                    if (!Objects.equals(deviceUa, userAgent)) {
                        ...
                        log.debug(
                                "User agent changed for device {}, expected {}, actual {}, revoking"
                                        + " device",
                    ...
                    return exchange.getSession()
                            .delayUntil(
                                    session -> {
                                        ...
                                        log.debug(
                                                "Session ID changed for device {}, deleting old"
                                                        + " session {}",
                                                existingDevice.getMetadata().getName(),
                                                sessionId);
                            ...
                            .doOnNext(
                                    session -> {
                                        ...
                                        existingDevice
                                                .getSpec()
                                                .setLastAccessedTime(session.getLastAccessTime());
                                        existingDevice
                                                .getSpec()
    ...
    record DeviceInfo(String browser, String os) {
        ...
        public static DeviceInfo parse(String agentString) {
            ...
            var browserVersion =
                    Stream.of(ua.major, ua.minor, ua.patch)
            ...
            var os = client.os;
            var osVersion =
                    Stream.of(os.major, os.minor, os.patch, os.patchMinor)
                            .filter(StringUtils::isNotBlank)
                            .collect(Collectors.joining("."));
