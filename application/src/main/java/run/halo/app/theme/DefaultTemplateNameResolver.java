...
/**
 * A default implementation of {@link TemplateNameResolver}, It will be provided for plugins to resolve template name.
 *
 * @author guqing
 * @since 2.11.0
 */
public class DefaultTemplateNameResolver implements TemplateNameResolver {
    ...
    public DefaultTemplateNameResolver(ViewNameResolver viewNameResolver, ApplicationContext applicationContext) {
    ...
    public Mono<String> resolveTemplateNameOrDefault(ServerWebExchange exchange, String name) {
        if (applicationContext instanceof PluginApplicationContext pluginApplicationContext) {
            ...
            return this.resolveTemplateNameOrDefault(exchange, name, pluginClassPathTemplate(pluginName, name));
        }
        return resolveTemplateNameOrDefault(exchange, name, pluginClassPathTemplate(SYSTEM_PLUGIN_NAME, name));
    }

    @Override
    public Mono<String> resolveTemplateNameOrDefault(ServerWebExchange exchange, String name, String defaultName) {
