...
/**
 * The {@link ArchiveRouteFactory} for generate {@link RouterFunction} specific to the template <code>posts.html</code>.
 *
 * @author guqing
 * @since 2.0.0
 */
...
public class ArchiveRouteFactory implements RouteFactory {
    ...
    HandlerFunction<ServerResponse> handlerFunction() {
        return request -> {
            ...
            return ServerResponse.ok()
                    .render(
                            templateName,
                            Map.of("archives", archivePosts(request), ModelConst.TEMPLATE_ID, templateName));
        };
    }
    ...
    private Mono<UrlContextListResult<PostArchiveVo>> archivePosts(ServerRequest request) {
        ...
        return configuredPageSize(environmentFetcher, SystemSetting.Post::getArchivePageSize)
                .flatMap(pageSize -> postFinder.archives(pageNum, pageSize, variables.getYear(), variables.getMonth()))
