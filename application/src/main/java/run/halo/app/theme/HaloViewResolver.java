...
public class HaloViewResolver extends ThymeleafReactiveViewResolver implements InitializingBean {
    ...
    public HaloViewResolver(
            FinderRegistry finderRegistry, ThymeleafProperties thymeleafProperties) {
    ...
    public static class HaloView extends ThymeleafReactiveView {
        ...
        @Autowired private TemplateEngineManager engineManager;

        @Autowired private ThemeResolver themeResolver;

        @Override
        public Mono<Void> render(
                Map<String, ?> model, MediaType contentType, ServerWebExchange exchange) {
                    ...
                    .flatMap(
                            theme -> {
                                ...
                                return super.render(model, contentType, exchange)
                                        .onErrorMap(
                                                TemplateProcessingException.class::isInstance,
                                                tee -> {
                                                    if (tee instanceof TemplateInputException) {
                                                        // map the error response exception while
                                                        // fragment not found
        ...
        protected Mono<Map<String, Object>> getModelAttributes(
            ...
            return Mono.deferContextual(
                    contextView ->
                            Flux.merge(modelAttributes, contextBasedStaticVariables)
                                    .collectList()
                                    .map(
                                            modelMapList -> {
