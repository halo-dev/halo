...
public class PostPublicQueryServiceImpl implements PostPublicQueryService {
    ...
    public Mono<ListResult<ListedPostVo>> list(ListOptions queryOptions, PageRequest page) {
        return postPredicateResolver
                ...
                .map(
                        option -> {
                            ...
                            if (fieldSelector != null) {
                                option.setFieldSelector(
                                        option.getFieldSelector().andQuery(fieldSelector.query()));
                            }
                            var labelSelector = queryOptions.getLabelSelector();
                            if (labelSelector != null) {
                                option.setLabelSelector(
                                        option.getLabelSelector().and(labelSelector));
                            }
                            return option;
                        })
                ...
                .flatMap(
                        list ->
                                convertToListedVos(list.getItems())
                                        .map(
                                                postVos ->
                                                        new ListResult<>(
                                                                list.getPage(),
    ...
    public Mono<List<ListedPostVo>> convertToListedVos(List<Post> posts) {
        ...
        var getContributors =
                contributorFinder.getContributors(userNames).collectMap(ContributorVo::getName);
        var getTags =
                tagFinder.getByNames(tagNames).collectMap(tagVo -> tagVo.getMetadata().getName());
        var getCategories =
                categoryFinder
                        .getByNames(categoryNames)
                                            ...
                                            post -> {
                                                ...
                                                var ghost =
                                                        requireNonNullElseGet(
                                                                contributors.get(GHOST_USER_NAME),
                                                                ContributorVo::ghost);
                                                vo.setOwner(
                                                        requireNonNullElse(
                                                                contributors.get(spec.getOwner()),
                                                                ghost));
                                                if (status != null
                                                        && !CollectionUtils.isEmpty(
                                                                status.getContributors())) {
                                                    vo.setContributors(
                                                            status.getContributors().stream()
                                                                    .map(
                                                                            name ->
                                                                                    requireNonNullElse(
                                                                                            contributors
    ...
    protected Mono<ContentVo> extendPostContent(Post post, ContentWrapper wrapper) {
        ...
        return extensionGetter
                ...
                .reduce(
                        Mono.fromSupplier(
                                () ->
                                        ReactivePostContentHandler.PostContentContext.builder()
                                                .post(post)
