...
public class SinglePageFinderImpl implements SinglePageFinder {
    ...
    public Mono<ListResult<ListedSinglePageVo>> list(Integer page, Integer size) {
        return singlePagePublicQueryService.listBy(new ListOptions(), PageRequestImpl.of(page, size));
    }

    Mono<Predicate<SinglePage>> queryPredicate() {
        Predicate<SinglePage> predicate = page ->
                page.isPublished() && Objects.equals(false, page.getSpec().getDeleted());
        Predicate<SinglePage> visiblePredicate =
                page -> Post.VisibleEnum.PUBLIC.equals(page.getSpec().getVisible());
        return currentUserName()
                .map(username -> predicate.and(visiblePredicate.or(
                        page -> username.equals(page.getSpec().getOwner()))))
