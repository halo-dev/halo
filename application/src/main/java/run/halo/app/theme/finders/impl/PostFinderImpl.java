...
class PostFinderImpl implements PostFinder {
                ...
                .flatMap(
                        predicate ->
                                client.get(Post.class, postName)
                                        ...
                                        .flatMap(
                                                post ->
                                                        postPublicQueryService.convertToVo(
                                                                post,
                                                                post.getSpec()
                                                                        .getReleaseSnapshot())));
    }
                            ...
                            return Mono.zip(
                                    ...
                                    (previous, next) ->
                                            NavigationPostVo.builder()
                                                    .previous(
                                                            previous.map(ListedPostVo::from)
                                                                    .orElse(null))
    ...
    private Mono<Post> findPreviousPost(Post currentPost) {
        ...
        return postPredicateResolver
                ...
                .flatMap(
                        listResult -> Mono.justOrEmpty(listResult.getItems().stream().findFirst()));
    }
    ...
    public Mono<ListResult<ListedPostVo>> list(Map<String, Object> params) {
        ...
        if (StringUtils.isNotBlank(query.getCategoryName())) {
            return listChildrenCategories(query.getCategoryName())
                    ...
                    .map(
                            categoryNames ->
                                    ListOptions.builder(query.toListOptions())
                                            .andQuery(in("spec.categories", categoryNames))
                                            .build())
                    .flatMap(
                            listOptions ->
                                    postPublicQueryService.list(
                                            listOptions, query.toPageRequest()));
        }
    ...
    @Override
    public Mono<ListResult<ListedPostVo>> listByCategory(
            Integer page, Integer size, String categoryName) {
        return listChildrenCategories(categoryName)
                ...
                .flatMap(
                        categoryNames -> {
                            ...
                            return postPublicQueryService.list(
                                    listOptions, getPageRequest(page, size));
                        });
    }
    ...
    @Override
    public Mono<ListResult<PostArchiveVo>> archives(
            Integer page, Integer size, String year, String month) {
        var listOptions = new ListOptions();
        ...
        return postPublicQueryService
                ...
                .map(
                        list -> {
                            Map<String, List<ListedPostVo>> yearPosts =
                                    list.get()
                                            .collect(
                                                    Collectors.groupingBy(
                                                            post ->
                                                                    HaloUtils.getYearText(
                                                                            post.getSpec()
                                                                                    .getPublishTime())));
                            List<PostArchiveVo> postArchives =
                                    yearPosts.entrySet().stream()
                                            .map(
                                                    entry -> {
                                                        String key = entry.getKey();
                                                        // archives by month
                                                        Map<String, List<ListedPostVo>> monthPosts =
                                                                                                ...
                                                                                                post ->
                                                                                                        HaloUtils
                                                                                                                .getMonthText(
                                                                                                                        post.getSpec()
                                                                                                                                .getPublishTime())));
                                                        // convert to archive year month value
                                                        // objects
                                                        List<PostArchiveYearMonthVo> monthArchives =
                                                                monthPosts.entrySet().stream()
                                                                        .map(
                                                                                monthEntry ->
                                                                                        PostArchiveYearMonthVo
                                                                                                .builder()
                                                                                                .posts(
                                                                                                        monthEntry
                                                                                                                .getValue())
                                            ...
                                            .sorted(
                                                    Comparator.comparing(PostArchiveVo::getYear)
                                                            .reversed())
                                            .toList();
                            return new ListResult<>(
                                    list.getPage(), list.getSize(), list.getTotal(), postArchives);
                        })
    ...
    public Mono<List<ListedPostVo>> random(int maxSize) {
        ...
        return postPredicateResolver
                ...
                .flatMap(
                        listOptions ->
                                client.countBy(Post.class, listOptions)
                                                ...
                                                total -> {
                                                    ...
                                                    var page =
                                                            RandomUtils.insecure()
                                                                    .randomInt(1, totalPages + 1);
                                                    var sort = defaultSort();
                                                    var firstRequest =
                                                            PageRequestImpl.of(
                                                                    page, effectiveSize, sort);
