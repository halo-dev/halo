...
public class ThemeResolver {
                                                        ...
                                                        themeState -> {
                                                            ...
                                                            var themeName =
                                                                    exchange.getRequest()
                                                                            ...
                                                                            .getFirst(
                                                                                    ThemeContext
                                                                                            .THEME_PREVIEW_PARAM_NAME);

                                                            if (StringUtils.isBlank(themeName)
                                                                    || !themeState
                                                                            .supportsPreviewTheme()) {
                                                            ...
                                                            var path =
                                                                    themeRoot
                                                                            .get()
                                                                            .resolve(themeName);
                                                            builder.name(themeName)
                                                                    .path(path)
                                                                    .active(active);

                                                            return client.fetch(
                                                                            Theme.class, themeName)
                                                                    .map(
                                                                            theme ->
                                                                                    builder.version(
                                                                                                    theme
    ...
    private Mono<ThemeActivationState> fetchActivationState() {
        ...
        var activatedMono =
                environmentFetcher
                        ...
                        .switchIfEmpty(
                                Mono.error(
                                        () -> new IllegalArgumentException("No theme activated")))
                        .doOnNext(builder::activatedTheme);

        var preivewDisabledMono =
                environmentFetcher
        ...
        var themeManageMono =
                ReactiveSecurityContextHolder.getContext()
                        ...
                        .flatMap(
                                au -> supportsPreviewTheme(authoritiesToRoles(au.getAuthorities())))
    ...
    @Builder
    record ThemeActivationState(
            String activatedTheme, boolean previewDisabled, boolean hasThemeManagementRole) {
